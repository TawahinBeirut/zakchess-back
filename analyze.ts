...
// Interface pour représenter chaque position à analyser
export interface ChessPosition {
  fen: string;
  playedMove: string;
  turn: 'w' | 'b';
  moveIndex: number; // Ajout d'un index pour identifier le coup
}

// Interface pour la réponse d'analyse complète
 ...
 */
function getCorrectPgn(pgn: string): string {
 ...
 */
export function convertPgnToPositions(pgn: string): ChessPosition[] {
  try {
 ...
 */
export async function callGpt(openai: OpenAI, position: ChessPosition): Promise<string> {
  try {
...
export function createInitRoute(openai: OpenAI) {
...
export function createAnalyzeMoveRoute(openai: OpenAI) {
  return async (req: Request, res: Response) => {
    if (!req.body) {
      return res.status(400).json({ error: 'Corps de requête vide ou invalide' });
    }
    ...
    try {
      ...
      const position: ChessPosition = { fen, playedMove, turn, moveIndex };
